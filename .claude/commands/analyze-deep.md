...
`$ARGUMENTS` がディレクトリの場合:
1. その中のデータファイル（`.csv`, `.tsv`, `.parquet`, `.json`）を列挙する
2. 出力ルートは `output/<ディレクトリ名>/` とする（例: `data/titanic/` → `output/titanic/`）
3. 各ファイルの出力先は `output/<ディレクトリ名>/<ファイル名（拡張子なし）>/` とする
4. **全ファイルの分析完了後**、ファイル間の関係性と全体サマリを `output/<ディレクトリ名>/summary.md` に出力する
...
- 最後にレポートのパス（ディレクトリ指定時はsummary.mdも含む）をユーザーに伝える
